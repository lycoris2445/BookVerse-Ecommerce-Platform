            ...
            {books.map((book) => (
              <div key={book.BookID || book.id} className="product-card" onClick={() => {
                ...
                <div className="product-info">
                  <h3 className="product-title">{book.Title || book.title}</h3>
                  <p className="product-author">Tác giả: {book.author || 'Chưa có thông tin'}</p>
                  <p className="product-price">{(book.Price || book.price || 0).toLocaleString('vi-VN')}₫</p>
                  {(book.Description || book.description) && (
                    <p className="product-description">{(book.Description || book.description).slice(0, 100)}...</p>
                  )}
...
// Mock BOOKS array for backward compatibility (empty - use API instead)
export const BOOKS = [];
